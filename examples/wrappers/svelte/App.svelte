...
<script lang="ts">
  ...
  const activeKind = 'svelte' as const;
  const config = WRAPPER_PAGE_CONFIG.svelte;
  const { agent, state: agentStateStore } = useQredexAgent(EXAMPLE_AGENT_CONFIG);
...
<main class="shell">
  <nav class="example-nav">
    ...
    <div class="example-link-row">
      {#each EXAMPLE_LINKS as link (link.kind)}
        <a class={getExampleLinkClass(link.kind, activeKind)} href={link.href}>{link.label}</a>
      {/each}
    </div>
  </nav>
  ...
  <section class="hero">
    <div>
      ...
      <div class="hero-notes">
        {#each config.heroNotes as note (note)}
          <span>{note}</span>
        {/each}
      </div>
    </div>
    <div class="hero-actions">
      <button class="btn btn-primary" id="set-intent-button" type="button" on:click={setRandomIntent}>
        Set Random IIT
      </button>
      <button class="btn btn-secondary" id="reload-button" type="button" on:click={reloadPage}>
        Refresh Page
      </button>
    </div>
  </section>
    ...
    <div class="stack">
      ...
      <section class="panel">
        <div class="panel-header">
          ...
          <span class="badge">Real Svelte app</span>
        </div>

        <div class="status-grid">
          <article class="metric">
            <span class="metric-label">IIT</span>
            <strong class="metric-value" id="status-iit">{snippetToken($agentStateStore.iit)}</strong>
          </article>
          <article class="metric">
            <span class="metric-label">PIT</span>
            <strong class="metric-value" id="status-pit">{snippetToken($agentStateStore.pit)}</strong>
          </article>
          <article class="metric">
          ...
          <article class="metric">
            ...
            <strong class="metric-value" id="status-cart-state">{$agentStateStore.cartState}</strong>
          </article>
        </div>
      ...
      <section class="panel">
        ...
        <div class="log" id="log">
          {#each snapshot.logs as entry (entry.id)}
            <div class={`log-entry log-${entry.level}`}>[{entry.time}] {entry.message}</div>
          {/each}
        </div>
      </section>
    </div>
    ...
    <aside class="side-stack">
      <section class="panel cart-panel">
        <div class="panel-header">
          ...
          <button class="btn btn-danger" id="empty-cart-button" type="button" on:click={emptyCart}>
            Empty cart
          </button>
        </div>
        ...
        <div class="cart-list" id="cart-list">
          ...
          {:else}
            {#each snapshot.lines as line (line.product.id)}
              <article class="cart-line">
                ...
                <div class="quantity-controls">
                  <button
                    ...
                    type="button"
                    on:click={() => decrementProduct(line.product.id)}
                  >
                    −
                  </button>
                  ...
                  <button
                    ...
                    type="button"
                    on:click={() => addProduct(line.product.id)}
                  >
                    +
                  </button>
                </div>
